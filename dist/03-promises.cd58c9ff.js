...
function onSubmitForm(evt) {
    ...
    if (delayStepForm >= 0 && firstDelayForm >= 0 && amountForm >= 0) setTimeout(()=>{
    ...
    else (0, _notiflixDefault.default).Notify.failure("Please fill numbers > 0 ");
    form.reset();
}
...
function updatePromise(x, y) {
    createPromise(x, y).then(({ i , delayStepForm  })=>{
        (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${i} in ${delayStepForm}ms`);
    }).catch(({ i , delayStepForm  })=>{
        (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${i} in ${delayStepForm}ms`);
    });
} //
